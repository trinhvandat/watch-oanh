...
/**
 * REST controller for managing {@link org.ptit.domain.Order}.
 */
@RestController
...
public class OrderResource {
    ...
    public List<Order> getAllOrders(@RequestParam(required = false) String filter) {
        if ("invoice-is-null".equals(filter)) {
            log.debug("REST request to get all Orders where invoice is null");
        ...
        if ("paymenttransaction-is-null".equals(filter)) {
            log.debug("REST request to get all Orders where paymentTransaction is null");
            return StreamSupport
                .stream(orderRepository.findAll().spliterator(), false)
                .filter(order -> order.getPaymentTransaction() == null)
                .toList();
        }
